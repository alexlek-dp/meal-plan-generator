...
def get_user_preferences():
    ...
    calories_per_day = float(
        input("Enter your daily calorie target (leave blank to use default): ") or 6000
    )
    carbs_ratio = float(
        input("Enter your carb percentage (leave blank to use default): ") or 0.5
    )
    fats_ratio = float(
        input("Enter your fat percentage (leave blank to use default): ") or 0.3
    )
    protein_ratio = float(
        input("Enter your protein percentage (leave blank to use default): ") or 0.2
    )
    ...
    meal_types_input = input(
        "Enter meal types (comma-separated, e.g., breakfast, lunch, dinner): "
    )
    meal_types = [
        meal_type_map.get(meal.strip().lower())
        for meal in meal_types_input.split(",")
        if meal_type_map.get(meal.strip().lower())
    ]
    days = int(
        input(
            "Do you want a one-day or weekly meal plan? (Enter 1 for one day, 7 for weekly, default: 7): "
        )
        or 7
    )
    ...
    intolerances_input = input(
        "Enter your food intolerances (comma-separated): "
    ).lower()
    intolerance_column_map = {
        "lactose intolerance": "Lactose Intolerance",
        "gluten intolerance": "Gluten Intolerance",
        "soy intolerance": "Soy Intolerance",
    ...
    selected_intolerances = [
        intolerance_column_map[i.strip()]
        for i in intolerances_input.split(",")
        if i.strip() in intolerance_column_map
    ]
...
def query_food_database(meal_types, intolerances):
    ...
    for intolerance in intolerances:
        filtered_recipes = filtered_recipes[
            filtered_recipes[intolerance] != 1
        ]  # Exclude recipes with intolerances
...
def structure_output(weekly_plan, user_preferences):
    ...
    for day_num, meal_plan in enumerate(weekly_plan, start=1):
        print(
            f"Day {day_num}'s Meal Plan (Target: {user_preferences['calories_per_day']} ± 100 calories):"
        )
        daily_calories = 0
        ...
        for recipe_index, recipe in enumerate(meal_plan):
            ...
            meal_type = user_preferences["meal_types"][
                recipe_index % len(user_preferences["meal_types"])
            ]
...
selected_recipes = query_food_database(
    user_preferences["meal_types"], user_preferences["intolerances"]
)
targets = calculate_macronutrient_targets(
    user_preferences["calories_per_day"],
    user_preferences["carbs_ratio"],
    user_preferences["fats_ratio"],
    user_preferences["protein_ratio"],
)
weekly_plan = generate_meal_plan(
    selected_recipes, targets, user_preferences["days"], user_preferences["meal_types"]
)
